...
class QueryGraphState(TypedDict):
    ...
    听书智库说明：
    - item_names 存放的是"书籍条目主体"（识别结果形如"三体-刘慈欣"），字段名沿用模板的 item_names；
    - reranked_docs 里除文本外还带书名 / 作者 / 内容类型 / 来源文件等元数据，供答案生成时引用来源。
    """
    ...
    hyde_doc: str  # HyDE 生成的假设性文档（便于调试与追溯）
    ...
    # 辅助信息
    item_names: List[str]  # 提取出的书籍主体名称
    rewritten_query: str  # 改写后的问题
    history: list  # 历史对话记录
    is_stream: bool  # 是否流式输出标记
    audio_url: str  # 语音提问的音频URL（随消息存历史，供刷新后回放；文本提问为空串）
    audio_text: str  # 语音提问的纯转写文本（上传文件场景 query 带提示词前缀时，用它存历史，避免污染对话）
