...
def step_2_rrf(source_weights, k, max_results):
    ...
    for chunks, weight in source_weights:
        ...
        for rank, chunk in enumerate(chunks, start=1):
            ...
            # 计算chunk_id所对应数据的分数
            score_map[chunk_id] = score_map.get(chunk_id, 0.0) + weight * (1.0 / (rank + k))
            # 存储chunk_id所对应的数据
...
def node_rrf(state: QueryGraphState):
    ...
    节点: RRF 融合 (node_rrf)
    实现内容:
    1. 统一实体格式（Milvus Hit 对象 / 字典 → 实体字典）；
    2. 仅合并"向量检索"与"HyDE 检索"两路，按 score(d)=Σ weight/(k+rank) 累加打分；
       网络搜索结果不参与融合，留到重排阶段再合并；
    3. 按综合得分降序，截断保留 Top-10。
    """
    # 记录当前任务的状态为进行中
