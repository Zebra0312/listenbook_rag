...
# 创建后台任务，即执行LangGraph的图对象完成内容导入流程
...
if __name__ == "__main__":
    # 说明：这里直接传入 app 对象而非 "模块:变量" 字符串，
    # 保证通过 `uv run python -m app.import_process.api.file_import_service` 也能正常启动
    # （字符串形式需要模块可被顶层导入，在 -m 方式下会 ImportError）。
